"""Bounded, proposal-only Hallam finalization using the validated refusal lesson."""
...
PACKET_ID = "FORGE-EXT-003"
MODEL = "deepseek-flash"
PRIOR_CUMULATIVE_ESTIMATE_USD = 0.40336
PRIOR_CUMULATIVE_MAX_USD = 0.46432
PACKET_ESTIMATE_CEILING_USD = 0.55408
CUMULATIVE_MAX_CAP_USD = 0.69648
...
FIELDS = frozenset({"include_industry"})
...
SAFE_STRATEGY = (
    "EXT-003 external proposal call with audited safe refusal evidence; model selects only "
    "decision=include_industry with positive_case=industry_only and "
    "negative_case=unrelated_industry; output is bounded to 1024 tokens; trusted code "
    "deterministically emits exactly two allowlisted Hallam file edits"
)
...
class FinalizeSelection(BaseModel):
    ...
    decision: Literal["include_industry"]
...
class RunResult:
    ...
    selection: FinalizeSelection | None
...
def _prompt(target: Path) -> str:
    parts = [
        "Return only decision=include_industry, positive_case=industry_only, and negative_case=unrelated_industry. Do not provide file content."
    ]
...
def _deterministic_edits(
    selection: FinalizeSelection, sources: dict[str, str]
) -> list[dict[str, str]]:
    if selection.decision != "include_industry":
        raise RefusalError("SELECTION_INVALID", "selection must include industry")
    if (
...
def run_once(
    ...
    *,
    ...
    request_maximum = _conservative_estimate(MAX_OUTPUT_TOKENS)
    cumulative_estimate = PRIOR_CUMULATIVE_ESTIMATE_USD + estimate
    cumulative_maximum = PRIOR_CUMULATIVE_MAX_USD + request_maximum
    if cumulative_estimate > PACKET_ESTIMATE_CEILING_USD:
        raise RuntimeError("Preflight refused: conservative spend exceeds packet cap")
    if cumulative_maximum > CUMULATIVE_MAX_CAP_USD:
        raise RuntimeError("Preflight refused: cumulative maximum exceeds owner cap")
    ...
    _append(
        ...
        "CALLING",
        {
            ...
            "packet_estimate_ceiling_usd": PACKET_ESTIMATE_CEILING_USD,
            "prior_cumulative_estimate_usd": PRIOR_CUMULATIVE_ESTIMATE_USD,
            "prior_cumulative_max_usd": PRIOR_CUMULATIVE_MAX_USD,
            "conservative_estimate_usd": estimate,
            "cumulative_conservative_estimate_usd": cumulative_estimate,
            "request_conservative_max_usd": request_maximum,
            "cumulative_conservative_max_usd": cumulative_maximum,
            "cumulative_conservative_max_cap_usd": CUMULATIVE_MAX_CAP_USD,
            "request_count": 1,
    ...
    error: str | None = None
    selection: FinalizeSelection | None = None
    ...
    try:
        ...
        try:
            selection = FinalizeSelection.model_validate_json(message)
